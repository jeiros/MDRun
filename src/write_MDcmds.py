def write_MDscript(job_number, json, timeList):
    ...
    with open("%s_job%s.pbs" % (json['simulation_details']['system_name'],
        ...
        if job_number == 1:
            ...
            file.write("/home/igould/pmemd.cuda_SPFP -O -i premin.in -o premin.out -c ${inpcrd} -p ${prmtop} -r premin.rst -ref ${inpcrd}\n\n")
            file.write("/home/igould/pmemd.cuda_SPFP -O -i sandermin1.in -o sandermin1.out -c premin.rst -p ${prmtop} -r sandermin1.rst\n\n")
            file.write("/home/igould/pmemd.cuda_SPFP -O -i 02_Heat.in -o 02_Heat.out -c sandermin1.rst -p ${prmtop} -r 02_Heat.rst -ref sandermin1.rst -x 02_Heat.nc\n\n")
            file.write("/home/igould/pmemd.cuda_SPFP -O -i 03_Heat2.in -o 03_Heat2.out -c 02_Heat.rst -p ${prmtop} -r 03_Heat2.rst -ref 02_Heat.rst -x 03_Heat2.nc\n\n")
            file.write("pbsexec -grace 15 /home/igould/pmemd.cuda_SPFP -O -i 05_Prod.in -o %s_${sim}ns.out -c 03_Heat2.rst -p ${prmtop} -r %s_${sim}ns.rst -x %s_${sim}ns.nc\n\n"
            ...
            file.write("cp %s/${prmtop} .\n\n" % json['simulation_details']['job_directory'])
            file.write("pbsexec -grace 15 /home/igould/pmemd.cuda_SPFP -O -i 05_Prod.in -o %s_${sim}ns.out -c ${prevrst} -p ${prmtop} -r %s_${sim}ns.rst -x %s_${sim}ns.nc\n\n"
                       % (json['simulation_details']['system_name'],
                          json['simulation_details']['system_name'],
                          json['simulation_details']['system_name']))
            file.write("cp /tmp/pbs.${PBS_JOBID}/%s_${sim}ns.rst %s/\n"
